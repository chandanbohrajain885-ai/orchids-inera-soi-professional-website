...
/**
 * Generic "team of people" section — renders a heading + a responsive grid of
 * member cards. Used by both CorePillars (InEra leadership) and
 * SOIFoundingTeam (School of Intelligence founding team) so both stay visually
 * consistent and share one implementation.
 */
export default function TeamSection({ members, eyebrow, accentClass = 'bg-neon-cyan', headingLine1, headingLine2, description }) {
    ...
    <section className="section-padding max-w-7xl mx-auto">
      ...
      <div className="text-center mb-14">
        <div className="inline-flex items-center gap-2 text-neon-cyan text-xs uppercase tracking-widest mb-3">
          <div className={`w-8 h-px ${accentClass}`} /> {eyebrow} <div className={`w-8 h-px ${accentClass}`} />
        </div>
        <h2 className="font-sora text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-4">
          {headingLine1}<br /><span className="blue-gradient">{headingLine2}</span>
        </h2>
      ...
      {/* Cards Grid */}
      <div className={`grid gap-6 ${members.length <= 2 ? 'sm:grid-cols-2 max-w-2xl mx-auto' : members.length === 3 ? 'sm:grid-cols-2 lg:grid-cols-3' : 'sm:grid-cols-2 xl:grid-cols-4'}`}>
